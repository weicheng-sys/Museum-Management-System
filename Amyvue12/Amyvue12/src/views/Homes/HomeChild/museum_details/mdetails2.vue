<template>
  <div>
    ...
    <img :src="list.mimg" style="width:100%;height:200px;" />
    ...
    <van-panel>
      ...
      <van-grid  :column-num="4">
        <van-grid-item v-for="(item,index) in mimglist" :key="index" v-if="index<4">
          <img :src="mimglist[index]" style="width:100px;height:100px;" />
        </van-grid-item>
      </van-grid>
    </van-panel>
    <!--视频面板-->
...
<script>
  export default {
    ...
    methods: {
      ...
      tocollection(){
        this.$router.push({name: 'collections', params: {mname: this.list.mname ,mid:this.list.mid}})
      },
      tovideo2(){
        this.$router.push({name: 'video2', params: {mname: this.list.mname}})
      },
      tovideo(){
        this.$router.push({name: 'mvideo', params: {museum: this.list}})
      },
      getid(){
      ...
      getlist(){
        ...
        _this.$axios.get('http://39.98.108.11:8081/video/findBymid?mid=' + _this.mid).then((results) => {
          ...
          console.log("博物馆编号" + _this.mid)

          console.log("findBymid" + JSON.stringify(results.data));
          // _this.videoList = results.data.results; //所有该博物馆的视频列表数据对应信息
          ...
          if(results.data.results.length==0)
          {
            ...
            results.data.results.forEach((e, i) => {
              ...
              if(e.status==1)
              {
                ...
                console.log(e.vid + e.vaddr);
                var url = e.vaddr;
                // url = url.substring(1, url.length - 4);
                url = url.substring(1, url.length);
                console.log(url);
                var urls = require('../../../../../static/' + url);
                var option = {
                  playbackRates: [0.7, 1.0, 1.5, 2.0], //播放速度
                  autoplay: false, //如果true,浏览器准备好时开始回放。
                  muted: false, // 默认情况下将会消除任何音频。
                ...
                this.$axios.get('http://39.98.108.11:8081/user/findById?uid=' + e.uid).then((results) => {
                  ...
                  if(results.data.results.icon==null)
                  {
                    ...
                    var imageurl = results.data.results.icon;
                    console.log(imageurl);
                    //console.log('sdfkdjfkdfjdskjfk'+require('C:/Users/惠普/Desktop/myvue/static/picture/20200507175520_kobe.jpg'));
                    var imagetype = imageurl.substring(results.data.results.icon.length - 3, results.data.results.icon.length);
                    imageurl = imageurl.substring(1, results.data.results.icon.length - 4);
                    if (imagetype == 'jpg')
                      _this.user.icon = require('../../../../../static/' + imageurl + '.jpg');
                    else if (imagetype == 'png') {
                      _this.user.icon = require('../../../../../static/' + imageurl + '.png');
                    }
                  }
                  ...
                  if (e.status == 0) {
                    ...
                    console.log('uuuuuuuuuuuuuuuuuuuuuuuu' + urls);
                    option.sources.src = urls;
                    console.log(option.sources.src);
                    // option.poster ='';
                  } else if (e.status == 1) {
                  ...
                  console.log("wahtkhsjhfj" + option.sources);
                  _this.playerOptions.push(option);
