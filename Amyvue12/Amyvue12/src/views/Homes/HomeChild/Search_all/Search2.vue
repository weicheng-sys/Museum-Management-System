      ...
      <van-cell v-for="(item,index) in list" :key="index" >
        <van-panel  @click='xiang(index)'>
          ...
          <van-row>
            ...
            <van-col span="8">
              <img :src=item.nimg style="width:100%;height:80px;" />
            </van-col>
          </van-row>
  ...
  export default {
    ...
    methods:{
      getParams:function(){
        ...
        var t = this
        this.$http.get('http://39.98.108.11:8081/News/findAll').then((res)=>{
          t.lists = res.data.result
          let l = t.lists.length;
          console.log(res.data.result)
          for(let i=0;i<l;i++){
            let x = t.lists[i].nimg;
            if(x.indexOf(',')){
              let y = x.indexOf(',')-3
              t.lists[i].nimg = x.substr(2,y);
              console.log(i+":"+t.lists[i].nimg)
            }
        ...
        this.$http.get('http://39.98.108.11:8081/News/findAll2').then((res)=>{
          t.lists = res.data.result
          let l = t.lists.length;
          console.log(res.data.result)
          for(let i=0;i<l;i++){
