...
def main():
    baseurl = "https://baike.baidu.com/item/%E5%AE%9D%E9%B8%A1%E9%9D%92%E9%93%9C%E5%99%A8%E5%8D%9A%E7%89%A9%E9%99%A2/4046043"
    # 1.爬取网页
...
def getData(baseurl):
    ...
    for item in soup.find_all('div', class_="basic-info cmn-clearfix"):
        ...
        #print(opentime)
    itemnumber="600余件(组)"
    itemtype="宝鸡地区考古发现的青铜器、玉器、陶器、金器等精品"

    for item in soup.find_all('div',class_="main-content"):
        item=str(item)
        ticket = re.findall(findTicket, item)[116]
        #print(ticket)
    data.append(name)
    data.append(place)
    data.append(type)
