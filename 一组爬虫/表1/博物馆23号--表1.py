...
def main():   #爬取数据，解析数据
    baseurl="https://baike.baidu.com/item/%E5%85%AB%E8%B7%AF%E5%86%9B%E5%A4%AA%E8%A1%8C%E7%BA%AA%E5%BF%B5%E9%A6%86"
...
def getdata(baseurl):
         ...
         # 2.逐一解析数据
            ...
            Position=re.findall(position,item)[1]   #提取博物馆位置
            Position=re.sub('\n', "", Position)  # 替换/
            data.append(Position)

            Type = re.findall(position, item)[7] #提取博物馆类别
            Type = re.sub('\n', "", Type)  # 替换/
            data.append(Type)
            ...
            ticket=re.findall(position,item)[5]  #提取博物馆门票
            ticket = re.sub('\n', " ", ticket)  # 替换/
            data.append(ticket)

            opentime=re.findall(position,item)[3]  #提取博物馆开放时间
            opentime = re.sub('\n', " ", opentime)  # 替换/
