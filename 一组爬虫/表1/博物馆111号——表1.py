...
def main():
    baseurl = "https://baike.baidu.com/item/%E8%A5%BF%E8%97%8F%E5%8D%9A%E7%89%A9%E9%A6%86"
    # 1.爬取网页
...
def getData(baseurl):
    ...
    for item in soup.find_all('div', class_="basic-info cmn-clearfix"):
        ...
        place = re.findall(findPlace, item)[3][61:64]+re.findall(findPlace, item)[3][68:76]
        #print(place)
    ...
    for item in soup.find_all('div',class_="main-content"):
        item=str(item)
        opentime=re.findall(findTicket,item)[40]+" "+re.findall(findTicket,item)[41]
        #print(opentime)
        ticket=re.findall(findTicket,item)[42][5:15]
        #print(ticket)
    data.append(name)
    data.append(place)
    data.append(type)
