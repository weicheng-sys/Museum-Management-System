...
def main():   #爬取数据，解析数据
    baseurl="https://baike.baidu.com/item/%E4%B8%AD%E5%9B%BD%E4%BA%BA%E6%B0%91%E9%9D%A9%E5%91%BD%E5%86%9B%E4%BA%8B%E5%8D%9A%E7%89%A9%E9%A6%86"
...
def getdata(baseurl):
         ...
         # 2.逐一解析数据
            ...
            Position=re.sub('\n', "", Position)  # 替换/
            Position = re.sub('<a href="/item/%E6%B5%B7%E6%B7%80%E5%8C%BA" target="_blank">', "",  Position)  # 去掉超链接
            Position = re.sub('</a>', "", Position)  #去掉超链接
            Position = re.sub('<a href="/item/%E5%A4%8D%E5%85%B4%E8%B7%AF" target="_blank">', "", Position)  # 去掉超链接
            Position = re.sub('</a>', "", Position)  # 去掉超链接
            ...
            ticket=re.findall(position,item)[8]  #提取博物馆门票
            ticket = re.sub('\n', "", ticket)  # 替换/
            data.append(ticket)
            ...
            opentime=re.sub('<sup class="sup--normal" data-ctrmap=":3," data-sup="3">',"",opentime)
            opentime = re.sub('</sup>', "", opentime)
            opentime = re.sub('\[3\]', "", opentime)#要转义[]
            opentime = re.sub('<a class="sup-anchor" name="ref__98534">', "", opentime)
            opentime=re.sub('</a>',"",opentime)
