...
def main():   #爬取数据，解析数据
    baseurl="https://baike.baidu.com/item/%E5%A4%A9%E6%B4%A5%E5%8D%9A%E7%89%A9%E9%A6%86"
...
def getdata(baseurl):
         ...
         # 2.逐一解析数据
            ...
            Type = re.findall(position, item)[3] #提取博物馆类别
            Type = re.sub('\n', "", Type)  # 替换/
            data.append(Type)
            ...
            opentime=re.findall(position,item)[8]  #提取博物馆开放时间
            opentime = re.sub('\n', " ", opentime)  # 替换/
            data.append(opentime)

            total=re.findall(position,item)[4]  #提取博物馆开放时间
            total = re.sub('\n', " ", total)  # 替换/
            data.append(total)
