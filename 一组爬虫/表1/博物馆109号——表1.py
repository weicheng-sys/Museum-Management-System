...
finTicket=re.compile(r'<dd class="basicInfo-item value">(.*?)</dd>',re.S)
findOpenTime=re.compile(r'<dd class="basicInfo-item value">(.*?)</dd>',re.S)
findItemNumber=re.compile(r'<dd class="basicInfo-item value">(.*?)</dd>',re.S)
findItemType=re.compile(r'<div class="para" label-module="para">(.*?)</div>',re.S)

def main():
    baseurl = "https://baike.baidu.com/item/%E9%87%8D%E5%BA%86%E7%BA%A2%E5%B2%A9%E9%9D%A9%E5%91%BD%E5%8E%86%E5%8F%B2%E5%8D%9A%E7%89%A9%E9%A6%86"
    # 1.爬取网页
...
def getData(baseurl):
    ...
    for item in soup.find_all('div', class_="basic-info cmn-clearfix"):
        ...
        ticket=re.findall(finTicket,item)[6]
        #print(ticket)

        opentime=re.findall(findOpenTime,item)[4]+"全年开放"
        #print(opentime)

        itemnumber=re.findall(findItemNumber,item)[7]
        #print(itemnumber)
    for item in soup.find_all('div',class_="main-content"):
        item=str(item)
        itemtype=re.findall(findItemType,item)[30][31:58]
