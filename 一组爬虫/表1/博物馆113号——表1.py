...
def main():
    baseurl = "https://baike.baidu.com/item/%E7%A7%A6%E5%A7%8B%E7%9A%87%E5%85%B5%E9%A9%AC%E4%BF%91%E5%8D%9A%E7%89%A9%E9%A6%86"
    # 1.爬取网页
...
def getData(baseurl):
    ...
    for item in soup.find_all('div', class_="basic-info cmn-clearfix"):
        ...
        type = re.findall(findType, item)[2][110:115]
        #print(type)
    ...
    for item in soup.find_all('div',class_="main-content"):
        item=str(item)
        opentime=re.findall(findOpenTime,item)[69]+re.findall(findOpenTime,item)[70][0:68]
        #print(opentime)

        ticket=re.findall(findTicket,item)[72][0:19]+" "+re.findall(findTicket,item)[72][138:203]
        #print(ticket)
    data.append(name)
    data.append(place)
    data.append(type)
