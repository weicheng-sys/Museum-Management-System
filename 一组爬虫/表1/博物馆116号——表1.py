...
def main():
    baseurl = "https://baike.baidu.com/item/%E8%A5%BF%E5%AE%89%E7%A2%91%E6%9E%97%E5%8D%9A%E7%89%A9%E9%A6%86"
    # 1.爬取网页
...
def getData(baseurl):
    ...
    for item in soup.find_all('div', class_="basic-info cmn-clearfix"):
        ...
        time = re.findall(findTime, item)[5]
        #print(time)
    itemtype="碑石、墓志"
    itemnumber="一千多块"
    ...
    for item in soup.find_all('div', class_="main-content"):
        item = str(item)
        opentime=re.findall(findOpentime,item)[31][0:155]
        opentime = re.sub('<br(\s+)?/>(\s+?)', " ", opentime)

        #print(opentime)

        ticket=re.findall(findTicket,item)[32][0:67]+" "+re.findall(findTicket,item)[32][182:]
        ticket = re.sub('<br(\s+)?/>(\s+?)', " ", ticket)
        #print(ticket)
