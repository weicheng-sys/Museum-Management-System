...
def main():
    baseurl = "https://baike.baidu.com/item/%E9%99%95%E8%A5%BF%E5%8E%86%E5%8F%B2%E5%8D%9A%E7%89%A9%E9%A6%86"
    # 1.爬取网页
...
def getData(baseurl):
    ...
    for item in soup.find_all('div', class_="basic-info cmn-clearfix"):
        ...
        place = "陕西省西安市雁塔区小寨东路91号"
        # print(place)

        time = re.findall(findTime, item)[4]
        #print(time)

        opentime=re.findall(findOpenTime,item)[5]
        #print(opentime)

        itemnumber=re.findall(findItemNumber,item)[7]
        #print(itemnumber)

        ticket=re.findall(findTicket,item)[9]
        #print(ticket)

    for item in soup.find_all('div',class_="main-content"):
        item=str(item)
        itemtype=re.findall(findItemType,item)[2][526:578]
        #print(itemtype)
    data.append(name)
    data.append(place)
    data.append(type)
