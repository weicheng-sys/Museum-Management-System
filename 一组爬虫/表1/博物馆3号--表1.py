...
def main():   #爬取数据，解析数据
    baseurl="https://baike.baidu.com/item/%E4%B8%AD%E5%9B%BD%E5%9C%B0%E8%B4%A8%E5%8D%9A%E7%89%A9%E9%A6%86"
...
def getdata(baseurl):
         ...
         # 2.逐一解析数据
            ...
            Position=re.findall(position,item)[5]   #提取博物馆位置
            Position=re.sub('\n', "", Position)  # 替换/
            Position = re.sub('<a data-lemmaid="10234" href="/item/%E8%A5%BF%E5%9F%8E%E5%8C%BA/10234" target="_blank">', "",  Position)  # 去掉超链接
            Position = re.sub('</a>', "", Position)  #去掉超链接
            data.append(Position)

            Type = re.findall(position, item)[1]#提取博物馆类别
            Type = re.sub('<a href="/item/%E5%8D%9A%E7%89%A9%E9%A6%86" target="_blank">', "", Type)  # 去掉超链接
            Type = re.sub('</a>', "", Type)  # 去掉超链接
            Type = re.sub('\n', "", Type)  # 替换/
            data.append(Type)

            data.append("")  #展区数量用空来代替

            Time=  re.findall(position, item)[2]#提取博物馆建馆时间
            Time = re.sub('\n', "", Time)  # 替换/
            data.append(Time)
            ...
            opentime=re.findall(position,item)[9]  #提取博物馆开放时间
            opentime= re.sub('\n', "", opentime)  # 替换/
