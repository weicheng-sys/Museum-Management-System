    ...
    export default {
        ...
        methods: {
            ...
            Delete(row) {
                ...
                //测试
                //console.log(row)
                _this.axios.get('/museum/news/delete?nid='+row.nid).then(function(resp){
            ...
            findBymname(search){
                ...
                this.currentpage=1
                this.pageshow=false
                this.axios.get('/museum/news/findBymname?mname='+search).then(function (response) {
                    console.log(response.data)
                     _this.allData=response.data
                    _this.length=response.data.length
                    console.log(_this.tableData)
                    //Vue.$set(_this,_this.allData,response.data)
                    //_this.allData=response.data
                    if(response.data.length>10)
                        _this.tableData=_this.allData.slice(0,9)
                     //  Vue.$set(_this,_this.tableData,_this.allData)
                    else
                     //   Vue.$set(_this,_this.tableData,_this.allData)
        ...
        created(){
            const _this=this
            //前后端对接 获取数据
            this.axios.get('/museum/news/findAll').then(function(response){
